...
const Footer = () => {
  return (
    <SectionContainer className="bg-VeryDarkViolet text-white py-14 flex justify-center items-center 
    flex-col gap-y-10 lg:flex-row lg:justify-between lg:items-start lg:gap-x-4">
      ...
      {footerData.map(({ titles }, index) => (
        <aside key={index} className="flex justify-center flex-col gap-8 lg:gap-16 lg:flex-row">
          {Object.keys(titles).map((sectionTitle, sectionIndex) => (
            <div key={sectionIndex}>
              ...
              {titles[sectionTitle].map((link, linkIndex) => (
                <span key={linkIndex} className="text-GrayishViolet cursor-pointer font-bold 
